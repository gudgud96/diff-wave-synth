...
tf.config.set_visible_devices([], 'GPU')    # temporary, need to find why this doesnt work
...
def preprocess(f, sampling_rate, block_size, signal_length, oneshot, 
    ...
    if os.path.exists(os.path.join(target_pitch_file, f.split("/")[-1].replace(".wav", "_pitch.npy"))):
        ...
        # pitch = pitch.reshape(x.shape[0], -1).squeeze().cpu().numpy()[:400]
...
if __name__ == "__main__":
    ...
    # y, pitch, loudness = preprocess(os.path.join(audio_path_prefix, audio_path[0]), 
    ...
    # print(y.shape, pitch.shape, loudness.shape)
